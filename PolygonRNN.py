...
angle = 1 - np.load('./Angle_Score.npy')
...
class PolygonRNN(object):
	...
	def AngleLoss(self, v_in, idx, seq_len):
		...
		p_0 = (tf.cast(tf.floor(idx_0 / self.v_out_res[0]), tf.int64), tf.mod(idx_0, v_out_res[0]))
		p_1 = (tf.cast(tf.floor(idx_1 / self.v_out_res[0]), tf.int64), tf.mod(idx_1, v_out_res[0]))
		p_2 = (tf.cast(tf.floor(idx_2 / self.v_out_res[0]), tf.int64), tf.mod(idx_2, v_out_res[0]))
		loss = 0.0
