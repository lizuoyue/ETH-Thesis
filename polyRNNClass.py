...
class PolyRNN(object):
	...
	def __init__(self, max_seq_len = 24, lstm_out_channel = [16, 8]):
		lstm_input_channel = [133] + lstm_out_channel[: -1]
		self.stacked_lstm = tf.contrib.rnn.MultiRNNCell(
			[self.ConvLSTMCell(in_c, out_c) for in_c, out_c in zip(lstm_input_channel, lstm_out_channel)]
		)
	...
	def CNN(self, img, boundary_true = None, vertices_true = None, reuse = None):
		feature = ModifiedVGG16(img, reuse)
		with tf.variable_scope(tf.get_variable_scope(), reuse = reuse):
		...
		loss = 0.0
		if boundary_true is not None and vertices_true is not None:
	...
	def Training(self, xx, bb, vv, yy, ee, ll):
		# Reshape inputs
		...
		initial_state = self.stacked_lstm.zero_state(BATCH_SIZE, tf.float32)
		...
		# FC Part
		outputs_reshape = tf.reshape(rnn_outputs, [-1, self.max_seq_len, 28 * 28 * self.lstm_out_channel[1]])
		with tf.variable_scope(tf.get_variable_scope(), reuse = reuse):
		...
		loss_RNN = tf.reduce_sum(tf.nn.softmax_cross_entropy_with_logits(labels = y_end_true, logits = logits)) / tf.reduce_sum(seq_len)
		return logits, loss_RNN
