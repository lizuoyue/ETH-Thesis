...
class HybridModel(object):
	...
	def RPN(self, img, reuse = None):
		...
		if self.mode == 'train':
			self.vgg16_res = vgg16_res
			return logit, delta
		else:
			return logit, delta, vgg16_res
	...
	def CNN(self, crop_info, vgg16_res = None, gt_boundary = None, gt_vertices = None, reuse = None):
		"""
		...
		"""
		if self.mode == 'train':
			feature = PolygonRNNFeature(self.vgg16_res, crop_info, reuse)
		else:
			assert(vgg16_res is not None)
			feature = PolygonRNNFeature(vgg16_res, crop_info, reuse)
	...
	def predict_rpn(self, aa, img_batch_size):
		...
		#
		pred_logit, pred_delta, vgg16_res = self.RPN(img, reuse = True)
		pred_pos_score = tf.nn.softmax(pred_logit)[..., 0]
		...
		return pred_score, pred_box, vgg16_res

	def predict_polygon(self, pp, vgg16_res):
		#
		crop_info = tf.reshape(pp, [-1, 5])
		pool2, pool3, _, conv4_3, conv5_3 = vgg16_res
		...
		vgg16_res = (pool2, pool3, None, conv4_3, conv5_3)

		#
		feature, v_first_with_prob = self.CNN(crop_info, vgg16_res, reuse = True)
