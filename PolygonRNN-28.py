...
class PolygonRNN(object):
	...
	def RNN(self, feature, v_in_true = None, rnn_out_true = None, seq_len = None, v_first = None, reuse = None):
		if not reuse:
			...
			v_in_true_0 = tf.tile(tf.reshape(v_in_true[:, 0, ...], [-1, 1, 28, 28, 1]), [1, self.max_seq_len, 1, 1, 1])
	...
	def Train(self, xx, bb, vv, ii, oo, ee, ll):
		...
		v_in_true     = tf.reshape(ii, [-1, self.max_seq_len, 28, 28, 1])
		v_out_true    = tf.reshape(oo, [-1, self.max_seq_len, 28 * 28])
		...
		v_out_pred = tf.reshape(rnn_pred[..., 0: 28 * 28], [-1, self.max_seq_len, 28, 28])
		end_pred = tf.reshape(rnn_pred[..., 28 * 28], [-1, self.max_seq_len, 1])
