...
if __name__ == '__main__':
	...
	start_by_city = {}
	for seed in [6671, 6678, 6679, 6687, 6708]:
		np.random.seed(seed)
		for city in cities:
	...
	for file in [
		...
		for city in cities:
			gt = gt_by_city[city]
			# dt = read_graph('segmentation/thr60/%s.pix.graph' % city)
			# dt = read_graph('out_deep/%s.fix.connected.graph' % city)
			# dt = read_graph('out_roadtracer/%s.0.3_0.3.newreconnect.graph' % city)
			# dt = read_graph('out_ours/%s_0.70.out.graph' % city)
			...
			end_choice = np.random.choice(len(end_pairs), 1000, replace = True)
			end_pairs = [end_pairs[i] for i in end_choice]

			temp_num = np.array([0, 0, 0, 0])
			for gts, dts, d_start in start_pairs:
				for gte, dte in end_pairs:
					d_end = gt.l2_dist(gt.v[gte], dt.v[dte])
					gtd = gt.spfa(gts)[gte] + (d_start + d_end) / 2
					dtd = dt.spfa(dts)[dte] + (d_start + d_end) / 2
					# print(d_start, d_end, gtd, dtd)
					# Error
					if gtd == 0:
						err = (1 - (dtd <= 128)) * 10
					else:
					...
					if gtd >= MAX_DIST:
						...
						if dtd >= MAX_DIST:
							...
							if abs(err) <= 5:
								...
								temp_num[1] += (err > 0)
								temp_num[2] += (err < 0)
			# print(city, temp_num / temp_num.sum())
			num += temp_num
		print(file, num / num.sum())
