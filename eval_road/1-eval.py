...
for dt in dts:
	...
	# if not (lurd[0] % 256 == 0 and lurd[1] % 256 == 0):
	# 	continue
	...
	vs = [(item[0] + round(lurd[0] / 256 * 28) + x_bias * 16 * 28, item[1] + round(lurd[1] / 256 * 28) + y_bias * 16 * 28) for item in dt['vertices']]
	r_idx2a_idx = {}
...
for k in res:
	...
	if False:
		...
		for s, t, score in e:
			if score > 0.75:
				draw.line([item * (256 / 28) + 4096 for item in v[s] + v[t]], width = 1, fill = 255)
		img.show()
		input()
		continue
	for th in np.arange(0.5, 1.0, 0.05):
		with open('./out_graph/%s_%.2lf.out.graph' % (k, th), 'w') as f:
			for item in v:
