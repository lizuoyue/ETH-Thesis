...
def FeaturePyramid(scope, mode, backbone_result, reuse = None):
	assert(mode == 'vgg' or mode == 'resnet')
	if mode == 'vgg':
		_, _, conv3_4, conv4_4, conv5_4 = backbone_result
		with tf.variable_scope(scope, reuse = reuse):
...
def AnchorLayer(scope, feature, anchors_per_pixel, reuse = None):
	"""
	...
	with tf.variable_scope(scope, reuse = reuse):
		...
		logit   = tf.layers.conv2d       (inputs = inter  , filters = 2*a, kernel_size = (1, 1), padding = 'same', activation = None)		#   ?
		delta   = tf.layers.conv2d       (inputs = inter  , filters = 4*a, kernel_size = (1, 1), padding = 'same', activation = None)		#   ?
