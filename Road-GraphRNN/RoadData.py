...
def getDataBatch(batch_size, mode, show = False):
	...
	while True:
		...
		if new_res[-1].shape[0] > 0:
			print(new_res[-1].shape[0], config.TRAIN_NUM_PATH)
...
if __name__ == '__main__':
	for i in range(1000):
		print(i)
		img, boundary, vertices, vertex_inputs, vertex_outputs, vertex_masks, seq_lens, seq_idx = getDataBatch(4, 'train', show = False)
