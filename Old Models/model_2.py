...
def model(x, b, v, y, end):
	...
	y_true = tf.reshape(y, [-1, 28, 28, 1, 7])
	end_true = tf.reshape(end, [-1, 1, 1, 1, 7])
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		...
		for i in range(n_iter):
			data = [utility.plotPolygon() for j in range(BATCH_SIZE)]
			polygon = [item[0] for item in data]
			img = [item[1] for item in data] # batch_size 224 224 3
			single = [item[2] for item in data] # batch_size num_vertices+1 28 28
			single_true = np.transpose(np.array(single), axes = [0, 2, 3, 1]) # batch_size 28 28 num_vertices+1
			end_true = np.array([[0,0,0,0,0,0,1] for i in range(BATCH_SIZE)]) # batch_size num_vertices+1
