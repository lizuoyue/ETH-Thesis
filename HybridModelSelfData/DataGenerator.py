...
class DataGenerator(object):
	def __init__(self, city_name, img_size, v_out_res, max_num_vertices, mode = None):
		self.img_size = img_size
		self.v_out_res = v_out_res
		self.max_num_vertices = max_num_vertices

		self.TRAIN_ANNOTATIONS_PATH = '/local/home/zoli/data/%s/%sBuildingTrain.json' % (city_name, city_name)
		self.VAL_ANNOTATIONS_PATH = '/local/home/zoli/data/%s/%sBuildingVal.json' % (city_name, city_name)
		self.TEST_ANNOTATIONS_PATH = '/local/home/zoli/data/%s/%sBuildingTest.json' % (city_name, city_name)

		self.TRAIN_IMAGES_DIRECTORY = '/local/home/zoli/data/%s/%sPatch' % (city_name, city_name)
		self.VAL_IMAGES_DIRECTORY = '/local/home/zoli/data/%s/%sPatch' % (city_name, city_name)
		self.TEST_IMAGES_DIRECTORY = '/local/home/zoli/data/%s/%sPatch' % (city_name, city_name)

		if mode == 'test':
			self.coco_test = COCO(self.TEST_ANNOTATIONS_PATH)
			self.TEST_IMAGE_IDS = self.coco_test.getImgIds(catIds = self.coco_test.getCatIds())
			self.TEST_CURRENT = 0
			self.TEST_FLAG = True
			self.TEST_RESULT = []
