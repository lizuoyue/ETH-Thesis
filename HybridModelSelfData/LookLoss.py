...
if __name__ == '__main__':
	# os.popen('scp leonhard:~/Master-Thesis/HybridModelResNet/LossTrain.out ./LossTrain.out')
	# os.popen('scp leonhard:~/Master-Thesis/HybridModelResNet/LossValid.out ./LossValid.out')
	# quit()
	...
	plt.plot((np.array(range(l_val)) + n_val) * 200, loss_cls_val, label = 'CLS Val')
	plt.plot((np.array(range(l_val)) + n_val) * 200, loss_dlt_val, label = 'DLT Val')
	plt.plot((np.array(range(l_val)) + n_val) * 200, loss_cnn_val, label = 'CNN Val')
	plt.plot((np.array(range(l_val)) + n_val) * 200, loss_rnn_val, label = 'RNN Val')
